    ...
    <div class="container">
        <div class="row">  
            <div class="leftform col-md-8">
                ...
                {!! Form::model($about, array('route' => array('aboutUpdate', $about->id), 'files'=>true)) !!}
                
                    <div class="form-group">
                        <label for="short">Описание</label> 
                        <textarea  class="form-control" id="short" name="short">{{ $about->short  }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="year">Год</label>
                        <input class="form-control" type="text" id="year" name="year" value="{{ $about->year }}">
                        
                        <input hidden="text" id="images" name="images" value="{{ $about->images }}">
                        <input hidden="text" id="images" name="newImage" value="{{ $imagePath = Session::get('image') }}">
                        <input hidden="text" id="id" name="id" value="{{ $about->id }}">
                    </div>
                    <div class="form-group">
                ...
                <label for="desc">Изображение</label>
                    ...
                    <div class="alert alert-success alert-block col">
                        ...
                        @empty(Session::get('success'))
                            <img class="imageUpload" style="width: 200px; " src="{{ asset('images/') }}/{{ $about->images }}">
                        @endempty
                        @if (count($errors) > 0)
